...
private theorem p8Gain_force_time_norm_bound
...
private theorem pressureP8_bound_sharp_core
    ...
    ENNReal.ofReal (r ^ (-4 / 3 : ℝ)) *
      ...
      ENNReal.ofReal (pressureP13Constant q *
  ...
  have hGbound : eLpNorm' G q (volume.restrict Tρ) ≤
    ...
    exact p8Gain_force_time_norm_bound hqpos hAforce hHs hHglobal
  ...
  have hfactor : (r / ρ) ^ (2 - 2 / q : ℝ) =
      (r / ρ) * (r / ρ) ^ (1 - 2 / q : ℝ) := by
    rw [show (2 - 2 / q : ℝ) = 1 + (1 - 2 / q) by ring]
    rw [Real.rpow_add (div_pos hr hρ)]
    rw [Real.rpow_one]
  let lamSharp : ℝ := (r / ρ) ^ (1 - 2 / q : ℝ) * lambda q f z ρ
  have hterm : A * (4 * Real.pi / 3) ^ (2 / 3 : ℝ) *
      r ^ (2 - 2 / q : ℝ) * ρ ^ (-2 + 2 / q : ℝ) * lambda q f z ρ =
        pressureP13Constant q * (r / ρ) * lamSharp := by
    dsimp [lamSharp]
    calc
      _ = (A * (4 * Real.pi / 3) ^ (2 / 3 : ℝ)) *
          (r ^ (2 - 2 / q : ℝ) * ρ ^ (-2 + 2 / q : ℝ)) *
            lambda q f z ρ := by ring
      _ = pressureP13Constant q * (r / ρ) ^ (2 - 2 / q : ℝ) *
          lambda q f z ρ := by rw [hconst, hpowscale]
      _ = pressureP13Constant q * (r / ρ) *
          ((r / ρ) ^ (1 - 2 / q : ℝ) * lambda q f z ρ) := by
        rw [hfactor]
        ring
  have hscale : ENNReal.ofReal (r ^ (-4 / 3 : ℝ)) *
      ((ENNReal.ofReal K) ^ (3 / 2 : ℝ) * volume Br *
        (∫⁻ s in Tr, ‖G s‖ₑ ^ (3 / 2 : ℝ))) ^ (2 / 3 : ℝ) ≤
      ENNReal.ofReal (pressureP13Constant q * (r / ρ) * lamSharp) := by
    calc
      _ ≤ ENNReal.ofReal (A * (4 * Real.pi / 3) ^ (2 / 3 : ℝ) *
          r ^ (2 - 2 / q : ℝ) * ρ ^ (-2 + 2 / q : ℝ) *
            lambda q f z ρ) := hscale0
      _ = ENNReal.ofReal
          (pressureP13Constant q * (r / ρ) * lamSharp) := by rw [hterm]
  have hcore := pressureP8_cylinder_bound
    (η := mollifiedBallCutoff z.1 hρ) (f := f) (x₀ := z.1)
    (t₀ := z.2) (r := r) (ρ := ρ) (K := K)
    (C₁₃ := pressureP13Constant q) (lam := lamSharp)
    hρ hr hhalf hK hGr hP8 (by simpa [Bρ, Br, Tr] using hscale)
  calc
    _ ≤ ENNReal.ofReal (pressureP13Constant q * (r / ρ) * lamSharp) := hcore
    _ = ENNReal.ofReal (pressureP13Constant q *
        (r / ρ) ^ (2 - 2 / q : ℝ) * lambda q f z ρ) := by
      congr 1
      dsimp [lamSharp]
      calc
        _ = pressureP13Constant q *
            ((r / ρ) * (r / ρ) ^ (1 - 2 / q : ℝ)) *
              lambda q f z ρ := by ring
        _ = pressureP13Constant q *
            (r / ρ) ^ (2 - 2 / q : ℝ) * lambda q f z ρ := by
...
/-- The sharp force-potential estimate in part (e) of `lem:pk-bounds`, together
with the separate-norm sum in `eq:p78-bound`. The coefficient is fixed from q
before the suitable solution and cylinder data. -/
theorem pressureP8_cylinder_bound_sharp_of_sws (q : ℝ) :
    ∃ C : ℝ, 0 ≤ C ∧
    ∀ {Ω : Set Vec3} {I : Set ℝ}
      {u : ParabolicPoint → Vec3} {Du : ParabolicPoint → Fin 3 → Vec3}
      {p : ParabolicPoint → ℝ} {f : ParabolicPoint → Vec3},
    ...
    ENNReal.ofReal (r ^ (-4 / 3 : ℝ)) *
        (eLpNorm' (fun w : ParabolicPoint =>
          ...
          eLpNorm' (fun w : ParabolicPoint =>
              pressureP8 (mollifiedBallCutoff z.1 hρ) f w.2 w.1) (3 / 2 : ℝ)
            (volume.restrict (parabolicCylinder z.1 z.2 r))) ≤
      ENNReal.ofReal (C * (r / ρ) * lambda q f z ρ) := by
  let C : ℝ := |lin34ForceCylinderConstant q| + |pressureP13Constant q|
  refine ⟨C, ?_, ?_⟩
  · change 0 ≤ |lin34ForceCylinderConstant q| + |pressureP13Constant q|
    positivity
  · intro Ω I u Du p f hsol z ρ r hρ hr hhalf hsub
    constructor
    · have hsharp := pressureP8_bound_sharp_core hsol hρ hr hhalf hsub
      have hP : pressureP13Constant q ≤ C := by
        dsimp [C]
        have h0 : 0 ≤ |lin34ForceCylinderConstant q| := abs_nonneg _
        have h1 : pressureP13Constant q ≤ |pressureP13Constant q| :=
          le_abs_self _
        linarith only [h0, h1]
      have hpow : 0 ≤ (r / ρ) ^ (2 - 2 / q : ℝ) :=
        Real.rpow_nonneg (by positivity) _
      have hlam : 0 ≤ lambda q f z ρ := by
        unfold lambda
        positivity
      have hcoeff : pressureP13Constant q *
          (r / ρ) ^ (2 - 2 / q : ℝ) * lambda q f z ρ ≤
          C * (r / ρ) ^ (2 - 2 / q : ℝ) * lambda q f z ρ :=
        mul_le_mul_of_nonneg_right
          (mul_le_mul_of_nonneg_right hP hpow) hlam
      exact hsharp.trans (ENNReal.ofReal_le_ofReal hcoeff)
    · have hsum := pressureP78_sum_of_norms_bound hsol hρ hr hhalf hsub
      have hC : lin34ForceCylinderConstant q ≤ C := by
        dsimp [C]
        have h0 : 0 ≤ |pressureP13Constant q| := abs_nonneg _
        have h1 : lin34ForceCylinderConstant q ≤
            |lin34ForceCylinderConstant q| := le_abs_self _
        linarith only [h0, h1]
      have hlam : 0 ≤ lambda q f z ρ := by
        unfold lambda
        positivity
      have hfactor : 0 ≤ (r / ρ) * lambda q f z ρ :=
        mul_nonneg (by positivity) hlam
      have hcoeff : lin34ForceCylinderConstant q *
          (r / ρ) * lambda q f z ρ ≤
          C * (r / ρ) * lambda q f z ρ := by
        calc
          _ = lin34ForceCylinderConstant q * ((r / ρ) * lambda q f z ρ) := by ring
          _ ≤ C * ((r / ρ) * lambda q f z ρ) :=
            mul_le_mul_of_nonneg_right hC hfactor
          _ = C * (r / ρ) * lambda q f z ρ := by ring
      exact hsum.trans (ENNReal.ofReal_le_ofReal hcoeff)
