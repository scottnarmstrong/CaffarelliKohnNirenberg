...
theorem singularSet_null_of_gradient_criterion
  ...
  have hcover : SingularSet Ω I u ⊆
      ⋃ z : A, SingularSet Ω I u ∩ U z.1 := by
    intro y hy
    have hyO : y ∈ spaceTimeSet Ω I := hy.1
    let y' : {z // z ∈ spaceTimeSet Ω I} := ⟨y, hyO⟩
    have hyV : y' ∈ ⋃ z ∈ A, V z := by
      rw [hAcov]
      trivial
    rcases mem_iUnion₂.1 hyV with ⟨z, hzA, hyVz⟩
    refine mem_iUnion.2 ⟨⟨z, hzA⟩, ?_⟩
    refine ⟨hy, ?_⟩
    exact hyVz
  apply le_antisymm
  · calc
      parabolicHausdorffMeasure 1 (SingularSet Ω I u) ≤
          parabolicHausdorffMeasure 1 (⋃ z : A,
            SingularSet Ω I u ∩ U z.1) := measure_mono hcover
      _ = 0 := measure_iUnion_null hSA
  · exact bot_le
