...
/-- The ball-power integral of a set indicator equals the integral restricted
to the intersection of the ball with the set. -/
theorem ballPowerIntegral_indicator {p : ℝ} (hp : 0 < p)
    {S : Set ParabolicPoint} (hS : MeasurableSet S)
    (g : ParabolicPoint → ℝ) (z : ParabolicPoint) (r : ℝ) :
    ballPowerIntegral p (S.indicator g) z r =
      ∫⁻ w in Metric.ball z r ∩ S, ENNReal.ofReal |g w| ^ p := by
  ...
  rw [ballPowerIntegral, heq, lintegral_indicator hS,
