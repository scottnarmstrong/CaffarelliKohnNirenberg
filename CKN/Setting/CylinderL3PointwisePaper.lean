...
private theorem source_integrable_sq_of_local_energy
...
private theorem source_integrable_sq_of_local_energy_prod
    {Ω' : Set Vec3} {J : Set ℝ} {E : Type}
    [NormedAddCommGroup E] {v : ParabolicPoint → E}
    (hv : AEStronglyMeasurable v (volume.restrict (spaceTimeSet Ω' J)))
    (hvlt : (∫⁻ z in spaceTimeSet Ω' J, ‖v z‖ₑ ^ (2 : ℝ)) < ⊤) :
  ...
  exact source_integrable_sq_of_local_energy hv hvlt

private theorem source_slice_data_ae
  ...
  rcases hdata with ⟨hu, hDu, _hess, henergy, hgrad⟩
  ...
  have hu_sq := source_integrable_sq_of_local_energy_prod hu hu_lt
  have hDu_sq := source_integrable_sq_of_local_energy_prod hDu hDu_lt
...
private theorem source_slice_energy_split {x : Vec3} {ρ r : ℝ}
  ...
  have hh : (∫ y in B, |E y - c|) ≤
    ...
    simpa only [Real.norm_eq_abs, abs_one, mul_one, Real.one_rpow, integral_const,
      smul_eq_mul,
      show (volume.restrict B).real univ = volume.real B by
...
private theorem source_energy_memLp_of_slice_data
    ...
    MemLp (fun y : Vec3 => vec3EuclideanNorm (u (y, s)) ^ 2)
      (ENNReal.ofReal (3/2 : ℝ)) (volume.restrict (vec3Ball z.1 ρ)) := by
...
/-- The pointwise two-radius cubic estimate under local energy and weak-gradient
data, with the compact-cylinder hypothesis ensuring the paper's slice quantity
is finite. -/
theorem cylinder_L3_pointwise_source
    {Ω : Set Vec3} {I : Set ℝ}
    {u : ParabolicPoint → Vec3} {Du : ParabolicPoint → Fin 3 → Vec3}
    (hΩ : IsOpen Ω) (hI : IsOpen I) (hIord : OrdConnected I)
    (hdata : ∀ Ω' J, localBox Ω I Ω' J →
  ...
  have hclosed : euclideanClosedBall z.1 R' ⊆ Ω := by
    ...
    simpa only [source_native_norm_eq] using hx'.trans_lt hR'R
  ...
  have hballR' : ∀ x ∈ vec3Ball z.1 R', x ∈ Ω' := by
    ...
    have hx' : x ∈ euclideanClosedBall z.1 R' := by
      ...
      simpa only [source_native_norm_eq] using hxe.le
    have hm : (x, z.2) ∈ K := by
  ...
  have hslicesJ := source_slice_data_ae hlocal
  ...
  have hpoint : ∀ᵐ s ∂volume.restrict Tρ,
    ...
    have hess : essSup
        ...
        (volume.restrict J) < ⊤ := hlocal.2.2.1
    have hessPoint : ∀ s,
      ...
      calc
        ...
        _ ≤ ∫⁻ y in vec3Ball z.1 ρ,
            3 * ‖u (y, s)‖ₑ ^ (2 : ℝ) :=
          lintegral_mono (fun y => source_ofReal_vec3Norm_sq_le (u (y, s)))
    ...
    have henergy : ∀ᵐ s ∂volume.restrict Tρ,
        (∫⁻ y in vec3Ball z.1 ρ,
          ENNReal.ofReal (vec3EuclideanNorm (u (y, s))) ^ (2 : ℝ)) ≤
            ENNReal.ofReal (ρ * alpha u z ρ ^ 2) := by
      filter_upwards [hEssBound] with s hs
    ...
    have hEmem := source_energy_memLp_of_slice_data hρ hball hs he
    ...
    have henergyMeas : AEStronglyMeasurable
      ...
      exact ((continuous_vec3EuclideanNorm.comp_aestronglyMeasurable
        ((hs.1.1.mono_measure
          (Measure.restrict_mono_set volume hballV)).aestronglyMeasurable)).pow 2)
    have hconvert :
        (∫⁻ y in vec3Ball z.1 ρ,
      ...
      calc
        ...
        _ = _ := by
          rw [Real.mul_rpow hρ.le (sq_nonneg _), ← Real.rpow_natCast,
            ← Real.rpow_mul ha]
          norm_num
    have hgradEq' := hgradEq s
    rw [hgradEq'] at hp
    have hsplit := source_slice_energy_split hρ hr hrρ hEmem
    have hvolC : (Real.pi * 4 / 3) ^ (1/3 : ℝ) ≤ 2 := by
      ...
      calc
        _ ≤ (poincareSobolevL1VectorConstant *
            ...
            (∫ y in vec3Ball z.1 ρ,
              ∑ i : Fin 3, vec3EuclideanNorm (Du (y, s) i) ^ 2) ^
                (1/2 : ℝ)) * (2 * ρ) := by
          apply mul_le_mul _ (mul_le_mul_of_nonneg_right hvolC hρ.le)
    ...
    have hinner :
        (∫ y in vec3Ball z.1 r, vec3EuclideanNorm (u (y, s)) ^ 2) ≤
          ...
          r ^ 3 / ρ ^ 2 * alpha u z ρ ^ 2 := by
      have hfirst' := hfirst
      rw [← hgradEq'] at hfirst'
      exact hsplit.trans (add_le_add hfirst' hsecond)
    exact slice_l3_real_bound_of_data hr hρ hrρ hball hs he hinner
  exact hpoint
